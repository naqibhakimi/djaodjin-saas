        ...
        with open(confpath) as conffile:
            ...
            while line != "":
                if not line.startswith("#"):
                    ...
                    if look:
                        value = look.group(2) % {"LOCALSTATEDIR": RUN_DIR + "/var"}
                        try:
...
load_config(
    os.path.join(os.getenv("TESTSITE_SETTINGS_LOCATION", RUN_DIR), "credentials")
)
    ...
    TEMPLATES = [
        {
            ...
            "DIRS": (
                os.path.join(BASE_DIR, "testsite", "templates", "jinja2"),
                os.path.join(BASE_DIR, "testsite", "templates"),
                os.path.join(BASE_DIR, "saas", "templates"),
            ),
            "OPTIONS": {"environment": "testsite.jinja2.environment"},
        }
    ]
